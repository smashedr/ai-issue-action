...
- name: 'AI Issue'
  ...
  with:
    model: gemini-2.5-flash # or any Claude, Gemini, OpenAI model
    instructions: 'You are an assistant responding to a GitHub Issue with Training Data:'
    file: docs/**/*.md
```

💡 Tip: Works with Gemini Free Tier [API Key](https://aistudio.google.com/app/api-keys).

## Features

- Works with Claude, Gemini, and OpenAI
- Provide Instructions via Text, File(s) or Both
- Add Custom Head or Tail Text to Reply
...
| [file](#file)                 |    -    |       -        | File(s) Glob Instructions |
